import { IsString, IsOptional, IsIn, IsObject, IsArray, ValidateNested } from 'class-validator';
import { ApiPropertyOptional } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import { CustomRoleDto } from '../../roles/dto/custom-role.dto';

/**
 * DTO for updating an existing tenant context
 */
export class UpdateTenantContextDto {
  @ApiPropertyOptional({
  ...
  @ApiPropertyOptional({
    description: 'Custom roles to apply (optional modifications to preset roles). If provided, these roles will be created/updated instead of the preset roles.',
    type: [CustomRoleDto],
  })
  @IsArray()
  @IsOptional()
