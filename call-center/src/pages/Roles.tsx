...
export function Roles() {
  ...
  const canManageRoles = user?.role === UserRole.ADMIN || user?.role === UserRole.TENANT_ADMIN;
    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Chargement des rôles...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {/* Total Roles */}
        <div className="rounded-lg bg-card dark:bg-card p-6 shadow">
        ...
        <div className="rounded-lg bg-card dark:bg-card p-6 shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Rôles Actifs</p>
        ...
        <div className="rounded-lg bg-card dark:bg-card p-6 shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-3xl font-bold text-purple-600 dark:text-purple-500">
        ...
        <div className="rounded-lg bg-card dark:bg-card p-6 shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Rôles Inactifs</p>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative flex-1 sm:max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-lg border border-input bg-background text-foreground py-2 pl-10 pr-4 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:bg-gray-800"
          />
        </div>
        ...
        <select
          ...
          className="rounded-lg border border-input bg-background text-foreground px-4 py-2 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:bg-gray-800"
        >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
        {filteredRoles.map((role) => (
          <div
            key={role.id}
            className="rounded-lg border border-border bg-card p-6 shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800"
          >
            {/* Header */}
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {role.canCallSameLevel && (
                  <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                    ➡️ Même niveau
                  </span>
                )}
                ...
                {role.canCallHigherLevel && (
                  <span className="inline-flex items-center rounded-full bg-purple-50 px-2 py-1 text-xs text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
            ...
            {/* Actions */}
            {canManageRoles && (
              <div className="mt-4 flex gap-2 border-t border-border pt-4 dark:border-gray-700">
                <button
                  onClick={() => setSelectedRole(role)}
                  className="flex-1 rounded-lg border border-indigo-600 px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"
                >
                  Modifier
                </button>
                <button
                  ...
                  className="flex-1 rounded-lg border border-red-600 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50"
                >
                  Supprimer
                </button>
              </div>
      ...
      {filteredRoles.length === 0 && (
        <div className="rounded-lg border-2 border-dashed border-border p-12 text-center dark:border-gray-700">
          <Shield className="mx-auto h-12 w-12 text-muted-foreground" />
          <h3 className="mt-4 text-lg font-medium text-foreground">Aucun rôle trouvé</h3>
          <p className="mt-2 text-sm text-muted-foreground">
            {searchTerm || selectedLevel
              ? 'Essayez de modifier vos critères de recherche.'
              : 'Commencez par créer votre premier rôle ou appliquer un preset.'}
          </p>
          {!searchTerm && !selectedLevel && canManageRoles && (
            <button
              onClick={() => setIsCreateModalOpen(true)}
